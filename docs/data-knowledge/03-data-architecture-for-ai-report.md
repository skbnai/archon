...
Working memory in the table above is often called *session data* in production systems — the short-lived state scoped to a single conversation or task run: the turn history, intermediate tool-call results, in-progress reasoning state, and any per-session user preferences. It differs from the other three memory types in one critical way: it is expected to be discarded, not accumulated.

Typical implementation splits session data across two tiers: an in-memory or low-latency store (Redis, an in-process cache, or a vector store scoped to the session ID) holding the active state for the duration of the interaction, and a checkpoint written to durable storage at defined intervals so a long-running or multi-turn session can be resumed after a process restart or an agent handoff. On session end (explicit close, TTL expiry, or task completion), the default behavior is eviction — anything worth keeping must be explicitly promoted into episodic or semantic memory rather than left in the session store, which most implementations do not guarantee durability for.

Two enterprise considerations follow directly from this: session data frequently contains the same PII present in the underlying conversation, so it needs its own retention policy rather than inheriting the (often longer) retention window of promoted long-term memory; and cross-session continuity — an agent task that pauses and resumes, or hands off between agent instances — requires explicit trace-context propagation across the session boundary, since most session stores treat each session as an isolated unit by default (see [End-to-End Lineage for AI-Era Data Systems](04-end-to-end-lineage-systems-report.md) for the tracing implications of this).
...
### Synthetic Data for AI Systems

Feature stores and knowledge graphs assume real production data exists to populate them. Three common situations break that assumption: a new domain with no historical data yet, a rare edge case too infrequent to collect enough real examples of, and a privacy or regulatory constraint that blocks using real records (PII, PHI, financial data) for training or evaluation at all. Synthetic data — records generated to match the statistical properties of real data without being copies of any real record — is the standard answer to all three.

**Generation approaches** fall into three tiers of fidelity and cost: rule-based simulation (fast, cheap, low fidelity — good for structural/schema testing); statistical and generative-model synthesis (GANs, diffusion models, or an LLM prompted against a real schema — moderate fidelity, the most common enterprise approach); and differential-privacy-preserving synthesis, which adds formal privacy guarantees at some cost to statistical fidelity, used when the source data is regulated. Enterprise tooling in this space includes Gretel, Mostly AI, Tonic.ai, and Synthesized, alongside in-house LLM-based generation pipelines.

**Validation is not optional.** Synthetic data that looks plausible but doesn't preserve the real data's distributional properties (correlations between fields, class imbalance, outlier frequency) produces models that fail in production despite passing training-time metrics. Standard practice is to validate synthetic data on downstream utility — train or evaluate on the synthetic set, then confirm performance transfers to a held-out real sample — not just on statistical similarity to the source.

**The emerging enterprise risk is model collapse**: training or evaluating successive model generations on data generated by prior models (rather than real data) can compound and amplify the errors and biases of the generating model. Enterprises using synthetic data at scale for training data augmentation — as opposed to one-off test/eval fixtures — need an explicit policy on how much synthetic vs. real data a training run may contain, and provenance tracking (see [End-to-End Lineage for AI-Era Data Systems](04-end-to-end-lineage-systems-report.md)) to know which is which downstream.
